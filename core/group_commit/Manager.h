...
public:
  ...
  Manager(std::size_t coordinator_id, std::size_t id, ContextType &context,
    ...
    worker_status.store(static_cast<uint32_t>(GCExecutorStatus::STOP));
  }
  ...
  void coordinator_start() {
    ...
    while (!stopFlag.load()) {
      ...
      signal_worker(GCExecutorStatus::START);
      ...
      // process replication
      n_completed_workers.store(0);
      set_worker_status(GCExecutorStatus::CLEANUP);
      wait_all_workers_finish();
      wait4_ack();
    }

    signal_worker(GCExecutorStatus::EXIT);
  }
  ...
  void non_coordinator_start() {
    ...
    for (;;) {
      ...
      GCExecutorStatus status = wait4_signal();
      if (status == GCExecutorStatus::EXIT) {
        set_worker_status(GCExecutorStatus::EXIT);
        break;
      }

      DCHECK(status == GCExecutorStatus::START);
      n_completed_workers.store(0);
      n_started_workers.store(0);
  ...
  void set_worker_status(GCExecutorStatus status) {
    worker_status.store(static_cast<uint32_t>(status));
  }

  void signal_worker(GCExecutorStatus status) {
  ...
  GCExecutorStatus wait4_signal() {
    ...
    return static_cast<GCExecutorStatus>(status);
  }
  ...
  void wait4_stop(std::size_t n) {
    ...
    for (auto i = 0u; i < n; i++) {
      ...
      CHECK(status == static_cast<uint32_t>(GCExecutorStatus::STOP));
    }
  }
  ...
  void broadcast_stop() {
    ...
    for (auto i = 0u; i < n_coordinators; i++) {
      ...
      ControlMessageFactory::new_signal_message(
          *messages[i], static_cast<uint32_t>(GCExecutorStatus::STOP));
    }
