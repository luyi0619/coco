...
TEST(TestSerialization, TestBasic) {
  ...
  EXPECT_EQ(scar::Deserializer<double>()(scar::Serializer<double>()(b), size),
            b);
...
TEST(TestSerialization, TestString) {
  ...
  EXPECT_EQ(scar::Deserializer<std::string>()(
                scar::Serializer<std::string>()(s), size),
            s);
...
TEST(TestSerialization, TestFixedString) {
  ...
  EXPECT_EQ(serializedString.length(),
            40 + sizeof(scar::FixedString<40>::size_type));
  EXPECT_EQ(scar::Deserializer<scar::FixedString<40>>()(serializedString, size),
            s);
