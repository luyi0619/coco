...
namespace scar {
  ...
  public:
    ...
    void set_read_validation_success_bit() {
      ...
      bitvec |= READ_VALIDATION_SUCCESS_BIT_MASK << READ_VALIDATION_SUCCESS_BIT_OFFSET;
    }
    ...
    bool get_read_validation_success_bit() const {
      return (bitvec >> READ_VALIDATION_SUCCESS_BIT_OFFSET) & READ_VALIDATION_SUCCESS_BIT_MASK;
    }

    // wts change in read validation bit
    void set_wts_change_in_read_validation_bit() {
      clear_wts_change_in_read_validation_bit();
      bitvec |= WTS_CHANGE_IN_READ_VALIDATION_BIT_MASK << WTS_CHANGE_IN_READ_VALIDATION_BIT_OFFSET;
    }

    void clear_wts_change_in_read_validation_bit() {
      bitvec &= ~(WTS_CHANGE_IN_READ_VALIDATION_BIT_MASK << WTS_CHANGE_IN_READ_VALIDATION_BIT_OFFSET);
    }

    bool get_wts_change_in_read_validation_bit() const {
      return (bitvec >> WTS_CHANGE_IN_READ_VALIDATION_BIT_OFFSET) & WTS_CHANGE_IN_READ_VALIDATION_BIT_MASK;
    }
  ...
  private:
     ...
     * read validation success bit (1) | wts change in read validation bit (1)
     * write lock bit(1) | read request bit (1) | local index read (1)  ]
     *
     * read validation success bit is set when read validation succeeds
     * wts change in read validation bit is set when wts changes,
     *  since delta may overflow in read validation rts extension
  ...
  public:
    ...
    static constexpr uint32_t WTS_CHANGE_IN_READ_VALIDATION_BIT_MASK = 0x1;
    static constexpr uint32_t WTS_CHANGE_IN_READ_VALIDATION_BIT_OFFSET = 3;
