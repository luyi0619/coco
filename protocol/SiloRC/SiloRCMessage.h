...
public:
  static std::size_t new_search_message(Message &message, ITable &table,
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(SiloRCMessage::SEARCH_REQUEST), message_size,
  ...
  static std::size_t new_lock_message(Message &message, ITable &table,
    ...
    auto message_size =
        MessagePiece::get_header_size() + key_size + sizeof(key_offset);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(SiloRCMessage::LOCK_REQUEST), message_size,
        table.tableID(), table.partitionID());
  ...
  static std::size_t new_abort_message(Message &message, ITable &table,
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(SiloRCMessage::ABORT_REQUEST), message_size,
        table.tableID(), table.partitionID());
  ...
  static std::size_t new_write_message(Message &message, ITable &table,
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(SiloRCMessage::WRITE_REQUEST), message_size,
        table.tableID(), table.partitionID());
  ...
  static std::size_t new_replication_message(Message &message, ITable &table,
    ...
    auto message_size = MessagePiece::get_header_size() + key_size +
                        field_size + sizeof(commit_tid);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(SiloRCMessage::REPLICATION_REQUEST), message_size,
        table.tableID(), table.partitionID());
...
public:
  static void search_request_handler(MessagePiece inputPiece,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(SiloRCMessage::SEARCH_REQUEST));
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(SiloRCMessage::SEARCH_RESPONSE), message_size,
  ...
  static void search_response_handler(MessagePiece inputPiece,
                                      ...
                                      Transaction *txn) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(SiloRCMessage::SEARCH_RESPONSE));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void lock_request_handler(MessagePiece inputPiece,
                                   ...
                                   Transaction *txn) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(SiloRCMessage::LOCK_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(SiloRCMessage::LOCK_RESPONSE), message_size,
        table_id, partition_id);
  ...
  static void lock_response_handler(MessagePiece inputPiece,
                                    ...
                                    Transaction *txn) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(SiloRCMessage::LOCK_RESPONSE));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void abort_request_handler(MessagePiece inputPiece,
                                    ...
                                    Transaction *txn) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(SiloRCMessage::ABORT_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void write_request_handler(MessagePiece inputPiece,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(SiloRCMessage::WRITE_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void replication_request_handler(MessagePiece inputPiece,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(SiloRCMessage::REPLICATION_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  get_message_handlers() {
    ...
    v.push_back(SiloRCMessageHandler::search_request_handler);
    v.push_back(SiloRCMessageHandler::search_response_handler);
    v.push_back(SiloRCMessageHandler::lock_request_handler);
    v.push_back(SiloRCMessageHandler::lock_response_handler);
    v.push_back(SiloRCMessageHandler::abort_request_handler);
    v.push_back(SiloRCMessageHandler::write_request_handler);
    v.push_back(SiloRCMessageHandler::replication_request_handler);
    return v;
  }
};
