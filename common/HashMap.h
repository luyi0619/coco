...
namespace scar {
    ...
    public:
        ...
        bool remove(const KeyType &key) {
            return _applyAt([&key](std::unordered_map<KeyType, ValueType> &map) {
        ...
        bool contains(const KeyType &key) {
            return _applyAt([&key](const std::unordered_map<KeyType, ValueType> &map) {
                return map.find(key) != map.end();
            }, bucketNo(key));
        }

        bool insert(const KeyType &key, const ValueType &value) {
            return _applyAt([&key, &value](std::unordered_map<KeyType, ValueType> &map) {
        ...
        ValueType &operator[](const KeyType &key) {
            return _applyAtRef([&key](std::unordered_map<KeyType, ValueType> &map) -> ValueType & {
                return map[key];
            }, bucketNo(key));
        }

        std::size_t size() {
            return _fold(0, [](std::size_t totalSize, const std::unordered_map<KeyType, ValueType> &map) {
                return totalSize + map.size();
            });
        }

        void clear() {
            _map([](std::unordered_map<KeyType, ValueType> &map) {
                map.clear();
            });
        }
    ...
    private:
        ...
        template<class ApplyFunc>
        auto _applyAt(ApplyFunc applyFunc, std::size_t i) {
            CHECK(i < N) << "index " << i << " is greater than " << N;
            locks_[i].lock();
            auto result = applyFunc(maps_[i]);
            locks_[i].unlock();
